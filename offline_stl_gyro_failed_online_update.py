...
def make_discrete_spec():
    ...
    If the installed rtamt package uses the alternate capitalization,
    fall back to that class only as a package-version compatibility correction.
    This does not change the STL formula, threshold, dataset, attack, or workflow.
    """
...
# ── 9. Evaluate robustness trace ────────────────────────────────────────────
def evaluate_trace(rx, ry, rz):
    spec = build_spec()
    ...
    for i in range(N):
        rho_i = spec.update(
            i,
            [
                ("gyro_residual_x", float(rx[i])),
                ("gyro_residual_y", float(ry[i])),
                ("gyro_residual_z", float(rz[i])),
            ],
        )
        rho_trace.append(rho_i)

    rho_trace = np.array(rho_trace, dtype=float)
    return rho_trace


rho_clean = evaluate_trace(
    gyro_residual_x_clean,
    gyro_residual_y_clean,
    gyro_residual_z_clean,
)

rho_attacked = evaluate_trace(
...
axes[2].plot(t_sec, rho_clean, label="ρ clean")
axes[2].plot(t_sec, rho_attacked, label="ρ attacked", linestyle="--")
...
if det_idx is not None:
    axes[2].axvline(
        t_sec[det_idx],
        linestyle=":",
        label=f"detection t={t_sec[det_idx]:.2f}s",
    )
