...
def evaluate_offline(rx, ry, rz):
    ...
    dataset = {
        "gyro_residual_x": list(zip(t_ms.tolist(), rx.astype(float).tolist())),
        "gyro_residual_y": list(zip(t_ms.tolist(), ry.astype(float).tolist())),
        "gyro_residual_z": list(zip(t_ms.tolist(), rz.astype(float).tolist())),
    }

    rho = spec.evaluate(dataset)
    rho = np.array(rho, dtype=float)

    # Expected offline format: rows [time_ms, robustness]
    rho_time_ms = rho[:, 0]
    rho_values = rho[:, 1]
