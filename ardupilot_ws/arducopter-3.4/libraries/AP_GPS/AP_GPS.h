...
public:
   ...
   enum GPS_Config {
    ...
    // Choi et al. RAID 2020 recovery (Alg.1 l.20): substitute the primary
    // GPS position/velocity with the software-sensor values (model states).
    void recovery_override(int32_t lat, int32_t lng, int32_t alt_cm,
                           const Vector3f &vel_ned) {
        state[primary_instance].location.lat = lat;
        state[primary_instance].location.lng = lng;
        state[primary_instance].location.alt = alt_cm;
        state[primary_instance].velocity = vel_ned;
    }
