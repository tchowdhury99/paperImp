...
void SoloGimbal_Parameters::set_param(gmb_param_t param, float value) {
    ...
    // mavlink_msg_param_set_send() always reads 16 bytes from param_id; copy into
    // a fixed 16-byte buffer so we never read past a shorter string literal
    char param_name[16] {};
    strncpy(param_name, get_param_name(param), sizeof(param_name) - 1);
    mavlink_msg_param_set_send(_chan, 0, MAV_COMP_ID_GIMBAL, param_name, _params[param].value, MAV_PARAM_TYPE_REAL32);
