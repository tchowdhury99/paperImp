...
public:
    ...
    // pressure with the software-sensor value and recompute altitude.
    void recovery_set_pressure(float pressure) {
        sensors[_primary].pressure = pressure;
        float altitude = get_altitude_difference(sensors[_primary].ground_pressure, pressure);
        if (!isnan(altitude) && !isinf(altitude)) {
            sensors[_primary].altitude = altitude + _alt_offset_active;
        }
    }
